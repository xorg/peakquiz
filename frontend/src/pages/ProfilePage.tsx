...
import type { ProfileStats, GameEntry, FavouritePeak } from "../types";
...
export function ProfilePage() {
  ...
  if (!stats)
    return (
      <main className={styles.page}>
        <p className={styles.empty}>{t("profileEmptyGuest")}</p>
      </main>
    );
  ...
  return (
    <main className={styles.page}>
      <div className={styles.header}>
        ...
        {editing ? (
          ...
          <button
            ...
            onClick={() => {
              setNicknameInput(stats.username);
              setEditing(true);
            }}
          >
      ...
      <div className={styles.statsRow}>
        ...
        <div className={styles.statCard}>
          <span className={styles.statValue}>
            {stats.correctGuesses.toLocaleString()}
          </span>
      ...
      <section className={styles.section}>
        <h2 className={styles.sectionTitle}>{t("favouritePeaksTitle")}</h2>
        {stats.favouritePeaks.length === 0 ? (
          <p className={styles.empty}>{t("favouritePeaksEmpty")}</p>
        ) : (
          <div className={styles.peaksGrid}>
            {stats.favouritePeaks.map((peak: FavouritePeak) => (
              <div key={peak.peakId} className={styles.peakCard}>
                ...
                <div className={styles.peakInfo}>
                  ...
                  <span className={styles.peakStat}>
                    {peak.correctCount} {t("correct")} · {peak.totalAttempts}{" "}
                    {t("attempts")}
                  </span>
                </div>
              </div>
            ...
            {stats.troublePeaks.map((peak) => (
              <div key={peak.peakId} className={styles.peakCard}>
                ...
                <div className={styles.peakInfo}>
                  <span className={styles.peakName}>{peak.peakName}</span>
                  <span className={styles.peakStat}>
                    {peak.wrongCount} {t("wrong")} · {peak.totalAttempts}{" "}
                    {t("attempts")}
                  </span>
                </div>
              </div>
              ...
              <div key={game.id} className={styles.gameRow}>
                ...
                <span className={styles.gameCounts}>
                  <span className={styles.gameCorrect}>
                    {game.correctCount} {t("gameCorrect")}
                  </span>
                  <span className={styles.gameSep}>·</span>
                  <span className={styles.gameWrong}>
                    {game.wrongCount} {t("gameWrong")}
                  </span>
                </span>
